-- Repair PostgreSQL JSONB indexing in Q20 summary for existing projects.
-- WITH ORDINALITY yields bigint; JSONB array indexes require integer.
-- Safe to run after all prior JIXGO migrations.
...
begin
  ...
  return jsonb_build_object(
    ...
    'bonusSummary', case when v_room.status::text = 'q20_summary' and v_question.kind = 'bonus' then jsonb_build_object(
      ...
      'perfect', (select count(*)::integer from answers a where a.room_id = v_room.id and a.question_id = v_question.id and not exists (select 1 from jsonb_array_elements(v_question.correct_answer->'answers') with ordinality expected(value, n) where coalesce(a.response->'answers'->>((n - 1)::integer), '') <> expected.value #>> '{}')),
      'rows', (select coalesce(jsonb_agg(jsonb_build_object('id', '20.' || n, 'correct', correct_count, 'wrong', wrong_count, 'unanswered', greatest(v_total - correct_count - wrong_count, 0)) order by n), '[]'::jsonb) from (select expected.n::integer as n, (select count(*)::integer from answers a where a.room_id=v_room.id and a.question_id=v_question.id and a.response->'answers'->>((expected.n - 1)::integer) = expected.value #>> '{}') as correct_count, (select count(*)::integer from answers a where a.room_id=v_room.id and a.question_id=v_question.id and btrim(coalesce(a.response->'answers'->>((expected.n - 1)::integer),'')) <> '' and a.response->'answers'->>((expected.n - 1)::integer) <> expected.value #>> '{}') as wrong_count from jsonb_array_elements(v_question.correct_answer->'answers') with ordinality expected(value,n)) rows)
    ) else null end
  );
end $$;
