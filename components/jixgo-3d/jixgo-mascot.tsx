...
export default function JixgoMascot({ mode = "idle", className = "", label = "Jixgo Fairytale Host" }: Props) {
  ...
  return <div className={`${styles.root} ${styles[mode]} ${className}`} role="img" aria-label={label}><Canvas dpr={[1, 1.5]} camera={{ position: [0, -.4, 6.4], fov: 38 }} gl={{ alpha: true, antialias: false, powerPreference: "low-power" }}><FairytaleHost mode={mode} /></Canvas></div>;
}
