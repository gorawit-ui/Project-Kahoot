...
function FairytaleHost({ mode }: { mode: JixgoMascotMode }) {
  ...
  useFrame(({ clock }) => {
    ...
    figure.current.rotation.y = Math.sin(seconds * .7) * (mode === "countdown" ? .035 : .08);
    figure.current.rotation.z = mode === "greeting" ? -.055 : Math.sin(seconds * 1.15) * .015;
  });
  const wave = mode === "greeting" ? -.95 : mode === "celebrate" ? -.62 : -.22;
  ...
  return <group ref={figure} position={[0, -1.35, 0]}>
    ...
    <group position={[0, .16, 0]}>
      ...
      <mesh position={[.02, .63, .31]}><boxGeometry args={[.9, 1.02, .08]} /><meshStandardMaterial color="#8d1838" roughness={.68} /></mesh>
      <group position={[-.56, .78, 0]} rotation={[0, 0, .48]}><mesh position={[0, -.24, 0]}><cylinderGeometry args={[.13, .15, .58, 12]} /><meshStandardMaterial color="#f1b592" /></mesh><mesh position={[0, .16, 0]}><sphereGeometry args={[.29, 14, 10]} /><meshStandardMaterial color="#173d8b" /></mesh><mesh position={[.06, -.58, 0]}><sphereGeometry args={[.14, 12, 10]} /><meshStandardMaterial color="#f1b592" /></mesh></group>
      <group position={[.57, .83, 0]} rotation={[0, 0, wave]}><mesh position={[0, -.28, 0]}><cylinderGeometry args={[.13, .15, .6, 12]} /><meshStandardMaterial color="#f1b592" /></mesh><mesh position={[0, .15, 0]}><sphereGeometry args={[.29, 14, 10]} /><meshStandardMaterial color="#173d8b" /></mesh><mesh position={[.06, -.61, 0]}><sphereGeometry args={[.14, 12, 10]} /><meshStandardMaterial color="#f1b592" /></mesh></group>
      <mesh position={[0, 1.49, 0]}><sphereGeometry args={[.43, 22, 16]} /><meshStandardMaterial color="#f4bd9e" roughness={.68} /></mesh>
      <mesh position={[0, 1.57, .02]} scale={[1.03, 1.08, 1]}><sphereGeometry args={[.45, 18, 14, 0, Math.PI * 2, 0, 1.72]} /><meshStandardMaterial color="#251d28" roughness={.82} /></mesh>
      <mesh position={[0, 1.9, 0]}><sphereGeometry args={[.09, 12, 8]} /><meshStandardMaterial color="#b51f3b" /></mesh><mesh position={[-.14, 1.93, 0]} rotation={[0, 0, .28]}><sphereGeometry args={[.17, 12, 8]} /><meshStandardMaterial color="#c72243" /></mesh><mesh position={[.14, 1.93, 0]} rotation={[0, 0, -.28]}><sphereGeometry args={[.17, 12, 8]} /><meshStandardMaterial color="#c72243" /></mesh>
      <mesh position={[-.13, 1.54, -.39]}><sphereGeometry args={[.032, 10, 8]} /><meshStandardMaterial color="#2a1b21" /></mesh><mesh position={[.13, 1.54, -.39]}><sphereGeometry args={[.032, 10, 8]} /><meshStandardMaterial color="#2a1b21" /></mesh><mesh position={[0, 1.36, -.4]} scale={[1.15, .3, 1]}><sphereGeometry args={[.08, 12, 6]} /><meshStandardMaterial color="#cf5a6e" /></mesh>
      <group position={[0, .97, -.55]}><mesh><sphereGeometry args={[.18, 18, 14]} /><meshStandardMaterial color="#d92036" metalness={.18} roughness={.22} /></mesh><mesh position={[0, .18, 0]}><cylinderGeometry args={[.022, .026, .12, 8]} /><meshStandardMaterial color="#6a4a20" /></mesh><pointLight intensity={mode === "countdown" || mode === "celebrate" ? 8 : 2.4} color="#ffdd75" distance={1.8} /></group>
    </group>
  </group>;
}
