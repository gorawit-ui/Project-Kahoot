...
export default function HostPage() {
  ...
  return <main className="page-shell"><section className="shell-content">
    ...
    <div className="host-grid"><div className="panel"><form className="form-stack" onSubmit={prepare}>
      ...
      <div className="control-grid"><button className="button primary" disabled={busy || !data} onClick={() => void command(position, "question")}>{game?.status === "question" ? "เริ่มเวลาใหม่" : position === 1 ? "เริ่มเกม" : "เริ่มข้อนี้"}</button><button className="button ghost" disabled={busy || !data || position >= 20} onClick={() => void command(position + 1, "question")}>ข้อต่อไป</button><button className="button ghost" disabled={busy || !data || game?.status !== "question"} onClick={() => void command(position, "paused")}>พักเกม</button><button className="button ghost" disabled={busy || !data || game?.status !== "question"} onClick={() => void command(position, "reveal")}>ปิดรับคำตอบ</button><button className="button ghost" disabled={busy || !data} onClick={() => void command(position, "finished")}>จบเกม</button><button className="button ghost" disabled={!data} onClick={() => setShowAll((value) => !value)}>{showAll ? "แสดง Top 5" : "แสดงทุกคน"}</button></div>
